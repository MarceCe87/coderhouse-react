...
const CartList = (state) => {
  ...
  const getTotalAmount = () => {
    let amount = 0;
    cartItems.map((item) => (amount += item.price * item.qty));
    return amount;
  };
      ...
      <Box>
        <List>
          {cartItems.length > 0 ? (
            ...
            <Box>
              <img className="photo" src={EmptyCart} alt="The Cart is Empty" />
            </Box>
          )}
          {cartItems.length > 0 ? (
            <Box>
              <Divider />
              <ListItemText
                sx={{ textAlign: "right", marginTop: 2, marginBottom: 2 }}
              >
                <Typography variant="h5" color={"#303ca9"}>
                  {cartItems.length > 0
                    ? `Total Amount: US$${getTotalAmount().toFixed(2)}`
                    : ""}
                </Typography>
                <Typography variant="subtitle2" color={"#303ca9"}>
                  or <b>6</b> installments of US${" "}
                  <b>{(getTotalAmount() / 6).toFixed(2)}</b>
                </Typography>
              </ListItemText>
              <Divider />
            </Box>
